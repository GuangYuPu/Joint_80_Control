        ...
        HMEMCPY(LocalAxes[AxisCnt].ObjDic,&DefCiA402AxisObjDic,SIZEOF(DefCiA402AxisObjDic));
        {
            ...
            while(pDiCEntry->Index != 0xFFFF)
            {
				...
				case 0x608A:
				     ...
				     break;
				case 0x6096:
                     pDiCEntry->pVarPtr = &LocalAxes[AxisCnt].Objects.objTargetKp;
				     break;
				case 0x6098:
                     pDiCEntry->pVarPtr = &LocalAxes[AxisCnt].Objects.objTargetKd;
                    break;
                case 0x60C2:
...
/////////////////////////////////////////////////////////////////////////////////////////
/**
 \brief    CiA402_DummyMotionControl用户电机控制接口/////
 \brief this functions provides an simple feedback functionality
*////////////////////////////////////////////////////////////////////////////////////////
INT32 i32TargetVelocity = 0;
INT32 i32TargetPosition = 0;
INT32 i32TargetTorque = 0;
INT32 i32Targetkp = 0;	
INT32 i32Targetkd = 0;
...
void CiA402_DummyMotionControl(TCiA402Axis *pCiA402Axis)
{
    ...
    pCiA402Axis->fCurPosition += ((double)pCiA402Axis->Objects.objVelocityActualValue) * IncFactor;//根据速度估算的位置
    pCiA402Axis->Objects.objPositionActualValue = 0;
            ...
            !pCiA402Axis->bBrakeApplied)
    {
                 ...
                 || pCiA402Axis->Objects.objPositionActualValue < pCiA402Axis->Objects.objTargetPosition))
        {
            ...
            switch(pCiA402Axis->Objects.objModesOfOperationDisplay)
            {
							...
							case INTERPOLATION_POSITION_MODE:  //主动僵硬待机
							{
            ...
            case CYCLIC_SYNC_POSITION_MODE:  //主动放松待机
						{
            ...
            case CYCLIC_SYNC_VELOCITY_MODE://转速模式
						{
                if(pCiA402Axis->i16State == STATE_OPERATION_ENABLED)
                {
                    ...
                    M0.controller=Type_velocity;//给电机控制器转速模式指令
                    i32TargetVelocity = pCiA402Axis->Objects.objTargetVelocity;
                    SpeedRef = (float)i32TargetVelocity * 0.0002604166666666f;//目标转速指令
                }
                else
								{
                    i32TargetVelocity = 0;
                ...
                break;
						...
						case CYCLIC_SYNC_TORQUE_MODE:
						{
							if(pCiA402Axis->i16State == STATE_OPERATION_ENABLED)
							{
								...
								motor_status = Running;
								M0.controller=Type_FPVc;

								i32TargetTorque=pCiA402Axis->Objects.objTargetTorque;
								i32Targetkp=pCiA402Axis->Objects.objTargetKp;
								i32Targetkd=pCiA402Axis->Objects.objTargetKd;
								i32TargetVelocity = pCiA402Axis->Objects.objTargetVelocity;
								i32TargetPosition = pCiA402Axis->Objects.objTargetPosition;
								
								
								M0.P_ang.ref = i32TargetPosition;//Q8
								M0.PID_vel.ref = i32TargetVelocity;//Q8
								FPV_KP = i32Targetkp;//Q14
								FPV_KD = i32Targetkd;//Q14
								TorqueRef = i32TargetTorque;//Q10
							}
							else
							{
								motor_status = Running;
...
//								motor_status = Standby;
		...
		pCiA402Axis->Objects.objVelocityActualValue = 614.4f * ENCOD_PLL.SPEED_ROTOR_FILT;//Q8
		
		/****位置传输****/
//		if(position > 180)
//		{
//			PosUpload = position - 360;
//		}
//		else
//		{
//			PosUpload = position;
//		}
		pCiA402Axis->Objects.objPositionActualValue = position * 256;//Q8
		/****位置传输****/

		pCiA402Axis->Objects.objTorqueActualValue = ActualTorque;//Q10
...
UINT16 APPL_GenerateMapping(UINT16* pInputSize,UINT16* pOutputSize)
{
    ...
    for(PDOAssignEntryCnt = 0; PDOAssignEntryCnt < sRxPDOassign.u16SubIndex0; PDOAssignEntryCnt++)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            /*drive mode supported    cspvt (cyclic sync position) : bit 7*/
            LocalAxes[PDOAssignEntryCnt].Objects.objSupportedDriveModes = 0x7FE;//0x380;
...
//                    pTmpData += SIZEOF(TCiA402PDO1A01);
			...
			*pTmpData++ = LocalAxes[AxisIndex].Objects.objVelocityActualValue &0xFFFF;
            *pTmpData++ = LocalAxes[AxisIndex].Objects.objVelocityActualValue>>16;
			*pTmpData++ = LocalAxes[AxisIndex].Objects.objTorqueActualValue & 0xFFFF;
            *pTmpData++ = LocalAxes[AxisIndex].Objects.objTorqueActualValue>>16;
			*pTmpData++ = LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay & 0xFF;;
...
//                pTmpData += SIZEOF(TCiA402PDO1601);
            ...
            LocalAxes[AxisIndex].Objects.objTargetPosition =SWAPWORD(*pTmpData++);
            LocalAxes[AxisIndex].Objects.objTargetPosition+=(SWAPWORD(*pTmpData++)<<16);
		    LocalAxes[AxisIndex].Objects.objTargetTorque =SWAPWORD(*pTmpData++);
            LocalAxes[AxisIndex].Objects.objTargetTorque+=(SWAPWORD(*pTmpData++)<<16);
			LocalAxes[AxisIndex].Objects.objTargetKp =SWAPWORD(*pTmpData++);
            LocalAxes[AxisIndex].Objects.objTargetKp+=(SWAPWORD(*pTmpData++)<<16);
			LocalAxes[AxisIndex].Objects.objTargetKd =SWAPWORD(*pTmpData++);
            LocalAxes[AxisIndex].Objects.objTargetKd+=(SWAPWORD(*pTmpData++)<<16);
			LocalAxes[AxisIndex].Objects.objTargetVelocity =SWAPWORD(*pTmpData++);
            LocalAxes[AxisIndex].Objects.objTargetVelocity+=(SWAPWORD(*pTmpData++)<<16);
		    LocalAxes[AxisIndex].Objects.objModesOfOperation = SWAPWORD(*pTmpData++)&0xff;
