...
float getAngle(MOTORController *M)
{
    ...
    {   //这个问题针对电机长时间连续一个方向转动
        M->full_rotation_offset = 0;              //速度模式，高速转动（大于200rad/s）时每次归零会导致电机抖动一次
        M->angle_prev = M->angle_prev - _2PI*2000;//同时控制2个电机，最大转速只能到T200，程序架构不合理
    }
